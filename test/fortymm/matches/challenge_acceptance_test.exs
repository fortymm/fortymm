defmodule Fortymm.Matches.ChallengeAcceptanceTest do
  use ExUnit.Case, async: true

  alias Fortymm.Matches.{Challenge, ChallengeAcceptance, Configuration}

  describe "changeset/2" do
    test "valid with pending challenge and different acceptor" do
      challenge = %Challenge{
        id: "abc123",
        created_by_id: 1,
        status: "pending",
    ...
    test "invalid when acceptor_id is missing" do
      challenge = %Challenge{
        id: "abc123",
        created_by_id: 1,
        status: "pending",
      ...
      assert "can't be blank" in errors_on(changeset).acceptor_id
    end

    test "invalid when acceptor_id is nil" do
      challenge = %Challenge{
        id: "abc123",
        created_by_id: 1,
        status: "pending",
    ...
    test "invalid when acceptor is the same as creator" do
      challenge = %Challenge{
        id: "abc123",
        created_by_id: 1,
        status: "pending",
      ...
      refute changeset.valid?
      assert "cannot accept your own challenge" in errors_on(changeset).acceptor_id
    end
    ...
    test "invalid with both acceptor same as creator and non-pending status" do
      challenge = %Challenge{
        id: "abc123",
        created_by_id: 1,
        status: "accepted",
    ...
    test "changeset stores acceptor_id correctly" do
      challenge = %Challenge{
        id: "store123",
        created_by_id: 100,
        status: "pending",
      ...
      assert changeset.valid?
      assert Ecto.Changeset.get_field(changeset, :acceptor_id) == 200
    end

    test "handles string acceptor_id by casting to integer" do
      challenge = %Challenge{
        id: "string123",
        created_by_id: 1,
        status: "pending",
      ...
      assert changeset.valid?
      assert Ecto.Changeset.get_field(changeset, :acceptor_id) == 2
    end

    test "invalid with invalid acceptor_id data type" do
      challenge = %Challenge{
        id: "invalid123",
        created_by_id: 1,
        status: "pending",
      ...
      changeset =
        ChallengeAcceptance.changeset(%ChallengeAcceptance{}, %{
          challenge: challenge,
          acceptor_id: "not_a_number"
        })

      refute changeset.valid?
      assert "is invalid" in errors_on(changeset).acceptor_id
    end
    ...
    test "validates acceptor cannot be zero matching creator id zero" do
      challenge = %Challenge{
        id: "zero123",
        created_by_id: 0,
        status: "pending",
    ...
    test "valid when acceptor id is different even with same digits" do
      challenge = %Challenge{
        id: "digits123",
        created_by_id: 123,
        status: "pending",
