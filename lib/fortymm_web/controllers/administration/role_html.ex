defmodule FortymmWeb.Administration.RoleHTML do
  @moduledoc """
  This module contains pages rendered by Administration.RoleController.
  """
  use FortymmWeb, :html
    ...
    <div class="card-body border-t border-base-300">
      <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
        ...
        <div class="join">
          <%= if @page > 1 do %>
            <a
              href={build_page_url(@conn, @page - 1, @sort_by, @sort_order, @search)}
          ...
          <%= for page_num <- page_range(@page, @total_pages) do %>
            ...
            <% else %>
              <a
                href={build_page_url(@conn, page_num, @sort_by, @sort_order, @search)}
                class="join-item btn btn-sm"
              >
                {page_num}
              </a>
  ...
  defp build_page_url(conn, page, sort_by, sort_order, search) do
    query_params = %{
