defmodule FortymmWeb.MatchHTML do
  @moduledoc """
  This module contains pages rendered by MatchController.
  """
  use FortymmWeb, :html
  ...
  def sort_link(assigns) do
    ~H"""
    <a
      href={build_sort_url(@conn, @field, @current_sort_by, @current_sort_order, @search, @status)}
  ...
  defp build_sort_url(conn, field, current_sort_by, current_sort_order, search, status) do
    ...
    query_params =
      if status && status != "" do
        Map.put(query_params, "status", status)
      else
        query_params
      end
    ...
    <div class="card-body border-t border-base-300">
      <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
        ...
        <div class="join">
          <%= if @page > 1 do %>
            <a
              href={build_page_url(@conn, @page - 1, @sort_by, @sort_order, @search, @status)}
          ...
          <%= for page_num <- page_range(@page, @total_pages) do %>
            ...
            <% else %>
              <a
                href={build_page_url(@conn, page_num, @sort_by, @sort_order, @search, @status)}
                class="join-item btn btn-sm"
              >
                {page_num}
              </a>
  ...
  defp build_page_url(conn, page, sort_by, sort_order, search, status) do
    query_params = %{
